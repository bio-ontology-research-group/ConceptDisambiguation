...
def refineVector(dictionary,sourceVector,firstVector,secondVector):
    indexesToDelete=[]
    wordsToDelete=[]
    for index,word in enumerate(sourceVector.term):
        flag = False
        #if (dWord for dWord in dictionary if (dWord != word.encode('utf-8'))):
        if any(word in dWord for dWord in dictionary):
            ...
            print "word founded in the dictionary: "+word.encode('utf-8')
    sourceVector = sourceVector.drop(sourceVector.index[indexesToDelete])
    print "Words to delete:"
    print " ".join(x for x in wordsToDelete)+"\n"
...
dictionary = loadDictionary("../resources/dictionary/used_terms.csv")
...
chemicalsVector,diseasesVector,genesVector = refineVector(dictionary,chemicalsVector,diseasesVector,genesVector);
diseasesVector,chemicalsVector,genesVector = refineVector(dictionary,diseasesVector,chemicalsVector,genesVector);
genesVector,diseasesVector,chemicalsVector = refineVector(dictionary,genesVector,diseasesVector,chemicalsVector);
